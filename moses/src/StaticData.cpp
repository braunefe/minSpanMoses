...
bool StaticData::LoadDecodeGraphs()
{
  ...
  //MSPnew : if min spans are specified, read them
  const vector<size_t> &minChartSpans = Scan<size_t>(m_parameter->GetParam("min-chart-span"));
  ...
  for(size_t i=0; i<mappingVector.size(); i++) {
    ...
    if (m_decodeGraphs.size() < decodeGraphInd + 1) {
      ...
      if (m_searchAlgorithm == ChartDecoding) {
        ...
        //MSPnew : construct DecodeGraph with minCharSpan
        size_t minChartSpan = (decodeGraphInd < minChartSpans.size()) ? minChartSpans[decodeGraphInd] : 0;
        decodeGraph = new DecodeGraph(m_decodeGraphs.size(), maxChartSpan, minChartSpan);
      } else {
