...
//MSPnew include PhraseDictionaryMinSpan
  ...
  m_alignmentsFile(alignmentsFile)
{
  ...
  //MSPnew : register SCFG_MIN_SPAN as thread safe dictionary
  if (implementation == Memory || implementation == SCFG || implementation == SuffixArray || implementation == SCFG_MIN_SPAN) {
...
PhraseDictionary* PhraseDictionaryFeature::LoadPhraseTable(const TranslationSystem* system)
{
  ...
  if (m_implementation == Memory) {
    ...
    CHECK(false);
#endif
  } //MSPnew : implementation of PhraseDictionarySCFG supporting min spans
	else if (m_implementation == SCFG_MIN_SPAN)
	{
		//memory phrase table
		VERBOSE(2,"using New Format phrase tables for large spans" << std::endl);
		if (!FileExists(m_filePath) && FileExists(m_filePath + ".gz")) {
			m_filePath += ".gz";
			VERBOSE(2,"Using gzipped file" << std::endl);
		}
		PhraseDictionaryMinSpan* pdm_ms  = new PhraseDictionaryMinSpan(m_numScoreComponent,this);
		bool ret = pdm_ms->Load(GetInput()
								 , GetOutput()
								 , m_filePath
								 , m_weight
								 , m_tableLimit
