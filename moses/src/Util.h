...
//!MSPnew : look if two integers are inside of an interval (clausal boundary constraint)
//FB : boundary 2 is a boundary marker, so the right side of the span has to be smaller
//FB : first boundary must always match
inline bool MatchesInterval(const int input, const int boundary)
{
    if( input == boundary)
    {
        return 1;
...
inline bool IsInside(const int input, const int boundary1, const int boundary2)
{
    if(boundary1 <= input && boundary2 >= input)
    {
        return 1;
    }
    else
